...
def main(url, save_dir):
    ...
    for n in c.datasets:
        ...
        with xr.open_dataset(ncml_url, mask_and_scale=False) as ds:
            ...
            # Deployment Time
            data_start = str(ds['time'].data.min())
            data_stop = str(ds['time'].data.max())
            ...
            for v in variables:
                ...
                if v in ref_des_dict[ds.stream]:
                    ...
                    available = False
                if ds[v].dtype == np.dtype('S64'): # this will skip most engineering/system variables because they are strings
                    data.append((ref_des, ds.stream, deployment, start_test, stop_test, dist_test, time_test,
                                 v, available, [], [], [], [], [],
                                 [], gap_list, [], [], []))
                    continue
                elif ds[v].dtype == np.dtype('datetime64[ns]'):
                    data.append((ref_des, ds.stream, deployment, start_test, stop_test, dist_test, time_test,
                                 v, available, [], [], [], [], [],
                                 [], gap_list, [], [], []))
                    continue
            ...
            df.to_csv(os.path.join(save_dir, '{}-{}-{}-{}-deployement_{}-current_time_{}.csv'.format(ds.subsite, ds.node, ds.sensor, ds.stream, deployment, dt.now().strftime('%Y-%m-%dT%H%M00'))), index=False)
